...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{x_sq} \hlkwb{<-} \hlkwa{function}\hldef{(}\hlkwc{x}\hldef{) x}\hlopt{^}\hlnum{2}
\hlkwd{integrate}\hldef{(x_sq,} \hlkwc{lower} \hldef{=} \hlnum{0}\hldef{,} \hlkwc{upper} \hldef{=} \hlnum{3}\hldef{)}
\end{alltt}
\begin{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{set.seed}\hldef{(}\hlnum{12345}\hldef{)}
\hldef{m} \hlkwb{<-} \hlnum{10000}
\hldef{X} \hlkwb{<-} \hlkwd{runif}\hldef{(}\hlkwc{n} \hldef{= m,} \hlnum{0}\hldef{,} \hlnum{3}\hldef{)}
\hlnum{3} \hlopt{*} \hlkwd{mean}\hldef{(}\hlkwd{x_sq}\hldef{(X))}
\end{alltt}
\begin{verbatim}
[1] 8.992983
\end{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{sqrt}\hldef{(}\hlnum{64.8}\hlopt{/}\hldef{m)}
\end{alltt}
\begin{verbatim}
[1] 0.08049845
\end{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{# Numeric Approximation:}
\hlkwd{sd}\hldef{(}\hlnum{3} \hlopt{*} \hldef{X}\hlopt{^}\hlnum{2}\hldef{)} \hlopt{/} \hlkwd{sqrt}\hldef{(m)}
\end{alltt}
\begin{verbatim}
[1] 0.07999898
\end{verbatim}
...
\begin{alltt}
\hldef{m} \hlkwb{<-} \hlnum{10000}
\hldef{X} \hlkwb{<-} \hlnum{3} \hlopt{*} \hlkwd{rbeta}\hldef{(}\hlkwc{n} \hldef{= m,} \hlnum{2}\hldef{,} \hlnum{1}\hldef{)}
\hldef{pi_x} \hlkwb{<-} \hlkwa{function}\hldef{(}\hlkwc{x}\hldef{)} \hlnum{2}\hlopt{*}\hldef{x}\hlopt{/}\hlnum{9}
\hlkwd{mean}\hldef{(}\hlkwd{x_sq}\hldef{(X)}\hlopt{/}\hlkwd{pi_x}\hldef{(X))}
\end{alltt}
\begin{verbatim}
[1] 8.993616
\end{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{sd}\hldef{( (X}\hlopt{^}\hlnum{2}\hldef{)} \hlopt{/} \hldef{(}\hlnum{2} \hlopt{*} \hldef{X} \hlopt{/} \hlnum{9}\hldef{) )} \hlopt{/} \hlkwd{sqrt}\hldef{(m)}
\end{alltt}
\begin{verbatim}
[1] 0.03181511
\end{verbatim}
...
\begin{alltt}
\hlcom{# Actual value}
\hlnum{1}\hlopt{-}\hlkwd{pnorm}\hldef{(}\hlnum{3}\hldef{)}
\end{alltt}
\begin{verbatim}
[1] 0.001349898
\end{verbatim}
...
\begin{alltt}
\hldef{m} \hlkwb{<-} \hlnum{10000}
\hldef{X1} \hlkwb{<-} \hlkwd{rnorm}\hldef{(}\hlkwc{n} \hldef{=} \hlnum{10000}\hldef{)}
\hldef{h} \hlkwb{<-} \hlkwa{function}\hldef{(}\hlkwc{x}\hldef{)} \hlkwd{ifelse}\hldef{(x} \hlopt{>} \hlnum{3}\hldef{,} \hlnum{1}\hldef{,} \hlnum{0}\hldef{)}

\hlkwd{mean}\hldef{(}\hlkwd{h}\hldef{(X1))}
\end{alltt}
\begin{verbatim}
[1] 0.0013
\end{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{sqrt}\hldef{(}\hlkwd{mean}\hldef{(}\hlkwd{h}\hldef{(X1))} \hlopt{*} \hldef{(}\hlnum{1} \hlopt{-} \hlkwd{mean}\hldef{(}\hlkwd{h}\hldef{(X1)))} \hlopt{/} \hldef{m)}
\end{alltt}
\begin{verbatim}
[1] 0.0003603207
\end{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{X2} \hlkwb{<-} \hlkwd{rnorm}\hldef{(m,} \hlkwc{mean} \hldef{=} \hlnum{4}\hldef{)}
\hldef{weights2} \hlkwb{<-} \hlkwd{dnorm}\hldef{(X2)} \hlopt{/} \hlkwd{dnorm}\hldef{(X2,} \hlkwc{mean} \hldef{=} \hlnum{4}\hldef{)}
\hlkwd{mean}\hldef{(}\hlkwd{h}\hldef{(X2)} \hlopt{*} \hldef{weights2)}
\end{alltt}
\begin{verbatim}
[1] 0.001326697
\end{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{sd}\hldef{(}\hlkwd{h}\hldef{(X2)}\hlopt{*}\hldef{weights2)} \hlopt{/} \hlkwd{sqrt}\hldef{(m)}
\end{alltt}
\begin{verbatim}
[1] 3.047903e-05
\end{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{X3} \hlkwb{<-} \hlnum{3} \hlopt{+} \hlkwd{rexp}\hldef{(m,} \hlnum{9}\hldef{)}
\hldef{weights3} \hlkwb{<-} \hlkwd{dnorm}\hldef{(X3)} \hlopt{/} \hlkwd{dexp}\hldef{(X3}\hlopt{-}\hlnum{3}\hldef{,} \hlnum{9}\hldef{)}
\hlkwd{mean}\hldef{(}\hlkwd{h}\hldef{(X3)} \hlopt{*} \hldef{weights3)}
\end{alltt}
\begin{verbatim}
[1] 0.001303543
\end{verbatim}
...
\hldef{est1} \hlkwb{<-} \hlkwd{cumsum}\hldef{(}\hlkwd{h}\hldef{(X1))} \hlopt{/} \hlnum{1}\hlopt{:}\hldef{m}
\hldef{est2} \hlkwb{<-} \hlkwd{cumsum}\hldef{(}\hlkwd{h}\hldef{(X2)} \hlopt{*} \hldef{weights2)} \hlopt{/} \hlnum{1}\hlopt{:}\hldef{m}
\hldef{est3} \hlkwb{<-} \hlkwd{cumsum}\hldef{(}\hlkwd{h}\hldef{(X3)} \hlopt{*} \hldef{weights3)} \hlopt{/} \hlnum{1}\hlopt{:}\hldef{m}
\hldef{all_vals} \hlkwb{<-} \hlkwd{data.frame}\hldef{(}
  \hlkwc{val} \hldef{=} \hlkwd{c}\hldef{(est1, est2, est3),}
  \hlkwc{method} \hldef{=} \hlkwd{rep}\hldef{(}\hlnum{1}\hlopt{:}\hlnum{3}\hldef{,} \hlkwc{each} \hldef{= m) |>} \hlkwd{factor}\hldef{(),}
  \hlkwc{m} \hldef{=} \hlkwd{rep}\hldef{(}\hlnum{1}\hlopt{:}\hldef{m,} \hlnum{3}\hldef{)}
\hldef{)}

\hlkwd{ggplot}\hldef{(all_vals |> dplyr}\hlopt{::}\hlkwd{filter}\hldef{(m} \hlopt{>} \hlnum{100}\hldef{),} \hlkwd{aes}\hldef{(}\hlkwc{x} \hldef{= m,} \hlkwc{y} \hldef{= val,} \hlkwc{col} \hldef{= method))} \hlopt{+}
  \hlkwd{geom_line}\hldef{()} \hlopt{+}
  \hlkwd{geom_hline}\hldef{(}\hlkwc{yintercept} \hldef{=} \hlnum{1}\hlopt{-}\hlkwd{pnorm}\hldef{(}\hlnum{3}\hldef{),} \hlkwc{linetype} \hldef{=} \hlsng{'dashed'}\hldef{)} \hlopt{+}
  \hlkwd{theme_bw}\hldef{()}
\end{alltt}
\end{kframe}

{\centering \includegraphics[width=\maxwidth]{tmp/figure/unnamed-chunk-17-1} 
...
{\centering \includegraphics[width=\maxwidth]{tmp/figure/unnamed-chunk-18-1} 
...
\begin{frame}[allowframebreaks]{Baseball statistics}
  \begin{itemize}
    \item One place that Hierarchical Bayes is particularly useful is for grouped data:
    \item Let $i = 1, 2, \ldots, k$ denote $k$ distinct groups, and $j = 1, 2, \ldots, n_i$ be the number of observations, per each group.
    \item Then, the data look like:
    $$
    Y_{1, 1}, \ldots, Y_{1, n_1}, Y_{2, 1}, \ldots, Y_{2, n_2}, \ldots Y_{k, n_k}
    $$
    \item We then have a natural Hierarchical structure to our model, where within each group $i$, the observations $1, \ldots, n_i$ are iid:
    $$
    Y_{i, j} \overset{\text{iid}}{\sim} f(y; \theta_i)
    $$
    \item Then, we create a prior for the group-parameters:
    $$
    \Theta_i \overset{\text{iid}}{\sim} \pi(\theta).
    $$
    \item This approach is extremely useful as a form of regularlization
  \end{itemize}
  ...
  \begin{exampleblock}{Baseball: Hitting Percentages}
    ...
    \mode<article>{
      \begin{itemize}
        \item From the percentages alone, Player B is a natural choice.
        \item What if we were told, however, that Player A had 1000 at bats, with success rate 353/1000, and Player B only has 10 at bats (successful 4/10 times)?
        \item From this example, there are two things to consider:
        \begin{enumerate}
          \item The best batting average of all time is: .3662 (Ty Cobb). Roughly only 200 players have hit over .3000 for their entire career, the average being somewhere closer to 0.2500. This information is very useful, and maybe should be accounted for.
          \item If there are many observations within a group, we should highly value these data, but if there are not many observations, we should ``penalize" towards the prior distribution.
        \end{enumerate}
        
        \item Continuing this example, we effectively have two binomial likelihoods. If $X_A$ and $X_B$ are the number of successful at-bats for players $A$ and $B$, respectively, then
        $$
        X_A \sim \text{Binom}(1000, p_A) \\
        X_B \sim \text{Binom}(10, p_B)
        $$
        \item For the hyper-prior of $P_i$, we will use a Beta distribution, since it's a convenient conjugate prior:
        $$
        P_i \sim Beta(\alpha, \beta).
        $$
        \item Now we need to pick the values of $\alpha, \beta$. We want a Beta with mean around $0.25$. Since the expected value is $\alpha / (\alpha + \beta)$, setting this equal to $1/4$ gives the relation: $3\alpha = \beta$.
        \item Using a simple normal approximation, we want roughly $0.25 + 2\sigma = 0.4$, so that the upper-end of this prior gives little-to-no weight for batting averages greater than $0.4$.
        Thus, we want $\sigma \approx 15/200$, which we can approximate with $2/25$.
        Using our relation $3\alpha = \beta$, this gives:
        \begin{align*}
          \Big(\frac{2}{25}\Big)^2 = \frac{\alpha\beta}{(\alpha+\beta)^(\alpha + \beta + 1)} = \frac{3\alpha^2}{16\alpha^2(4\alpha + 1)},
        \end{align*}
        which implies $\alpha \approx 7$.
        \item Thus, the hyper-prior we will use is $\text{Beta}(7, 21)$, shown in Figure~\ref{fig:battingPrior}
      \end{itemize}
      
      \begin{figure}[ht]
        \includegraphics[width=\textwidth]{figures/beta_prior_plot.png}
        \caption{\label{fig:battingPrior}Prior distribution, Beta$(7, 21)$ for the batting percentages.}
      \end{figure}
     
     \begin{itemize}
      \item Recall that for the Beta-Binomial conjugate distributions, the posterior for the probability of success is given by:
      $$
      P_i | X_i = x_i \sim \text{Beta}(\alpha + x_i, \beta + n_i - x_i).
      $$
     
      \item For the current example, of the two-baseball players, we get posterior distributions:
      \begin{align*}
      P_A | X_A = x_A &\sim \text{Beta}(7 + 353, 21 + 647) \\
      P_B | X_B = x_B &\sim \text{Beta}(7 + 4, 21 + 6).
      \end{align*}
      
      The posterior means for each of these distributions are $0.350$ and $0.289$, respectively. The respective posteriors are plotted in Figure~\ref{fig:baseballPost}.
      
      \end{itemize}
      \begin{figure}[ht]
        \includegraphics[width=\textwidth]{figures/baseballPost.png}
      \caption{\label{fig:baseballPost}Posterior distribution estimates for the estimated batting percentage of the two Baseball players. Player A is in blue, and Player B is in Red. For refefence, the prior distribution is given as a dashed black line.}
      \end{figure}
...
\begin{frame}[allowframebreaks]{Empirical Bayes}
  ...
  \begin{exampleblock}{Baseball Players}
