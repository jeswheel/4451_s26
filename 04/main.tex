...
\begin{frame}[allowframebreaks]{Variance reduction techniques}
  \begin{itemize}
  ...
  \begin{block}{Antithetic Variates Method}
    The next approach is similar, but gives a way of simulating negatively correlated random variables.
    
    In the simplest case, suppose we want to compute an integral
    $$
    \int_0^1 f(x) dx.
    $$
    We can sample $U_i \sim U(0, 1)$, and do Monte-Carlo as usual with $f(U_i)$. However, if $f$ is monotonic, then $U'_i = 1-U_i$ is antithetic to $U_i$, and 
    $$
