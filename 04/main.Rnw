...
\begin{frame}[allowframebreaks]{Hierarchical Bayes}
  ...
  \begin{itemize}
    ...
    \item The prior itself depends on parameters, say $\pi(\theta;\psi)$.
    \item How do we choose $\psi$? Why don't we put a prior on these as well?
    ...
    \item Philosophically, this situation naturally arises if we want to pick a conjugate prior for $\Theta$, but are not committal about the \alert{hyperparameters} $\Psi$ that define the distribution of $\Theta$. Then, $\Psi$ should also be modeled as a random variable.

    \framebreak

    \item This leads to a hierarchical model:
    $$
    ...
    \item Why would we want to do this? Do we need a prior now hyper-parameters for $\psi$?
    \begin{enumerate}
      \item We'll see that this is sometimes a very useful way of thinking about a problem.
      \item We could, but this is not usually useful.
    \end{enumerate}
...
\begin{exampleblock}{Trivial case: hierarchical Normal-Normal}
  ...
  \mode<article>{
    \begin{itemize}
      \item This model seems rather odd, unless there is a good reason to get the posterior $(\Theta, M) | X = x^*$.
      \item Otherwise, we can show that the \emph{hyperparameter} $M$ can be eliminated from the model.
      \item In particular, consider the marginal distribution of $\Theta$:
      $$
      \pi_\Theta(\theta) = \int \pi_{\Theta | M}(\theta | \mu) \pi_M(\mu) \, d\mu,
      $$
      \item Some algebra (i.e., completing the square in the exponential terms), shows that this implies: $\Theta \sim N(0, 2)$.
