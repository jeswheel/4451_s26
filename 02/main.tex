...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{set.seed}\hldef{(}\hlnum{123}\hldef{)}  \hlcom{# Reproducible results}
\hldef{x} \hlkwb{<-} \hlkwd{rpois}\hldef{(}\hlkwc{n} \hldef{=} \hlnum{25}\hldef{,} \hlkwc{lambda} \hldef{=} \hlnum{6}\hldef{)}
\hlkwd{plot}\hldef{(}\hlkwd{ecdf}\hldef{(x))}
\hlkwd{lines}\hldef{(}
  \hlkwc{x} \hldef{=} \hlkwd{seq}\hldef{(}\hlnum{1e-16}\hldef{,} \hlnum{15}\hldef{,} \hlkwc{length.out} \hldef{=} \hlnum{1000}\hldef{),}
  \hlkwc{y} \hldef{=} \hlkwd{ppois}\hldef{(}\hlkwd{seq}\hldef{(}\hlnum{1e-16}\hldef{,} \hlnum{15}\hldef{,} \hlkwc{length.out} \hldef{=} \hlnum{1000}\hldef{),} \hlnum{6}\hldef{),}
  \hlkwc{col} \hldef{=} \hlsng{'red'}
\hldef{)}
\end{alltt}
\end{kframe}

{\centering \includegraphics[width=\maxwidth]{tmp/figure/unnamed-chunk-8-1} 
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{sqrt}\hldef{(}\hlkwd{mean}\hldef{(x)} \hlopt{/} \hlkwd{length}\hldef{(x)) |>} \hlkwd{round}\hldef{(}\hlnum{2}\hldef{)}
\end{alltt}
\begin{verbatim}
[1] 1.04
\end{verbatim}
...
\hldef{lambda} \hlkwb{<-} \hlkwd{mean}\hldef{(x)}
\hldef{results} \hlkwb{<-} \hlkwd{replicate}\hldef{(}\hlnum{10}\hldef{,} \hlkwd{rpois}\hldef{(}\hlkwd{length}\hldef{(x),} \hlkwc{lambda} \hldef{= lambda)) |>}
  \hlkwd{as.data.frame}\hldef{()}
\hlkwd{colnames}\hldef{(results)} \hlkwb{<-} \hlkwd{paste0}\hldef{(}\hlsng{"sample_"}\hldef{,} \hlnum{1}\hlopt{:}\hlnum{10}\hldef{)}
\hldef{results |>}
  \hlkwd{pivot_longer}\hldef{(}
    \hlkwc{cols} \hldef{=} \hlkwd{everything}\hldef{(),} \hlkwc{names_to} \hldef{=} \hlsng{"replicate"}\hldef{,}
    \hlkwc{names_prefix} \hldef{=} \hlsng{"sample_"}\hldef{,} \hlkwc{values_to} \hldef{=} \hlsng{"val"}
  \hldef{)} \hlopt{%>%}
  \hlkwd{ggplot}\hldef{(}\hlkwd{aes}\hldef{(val,} \hlkwc{group} \hldef{= replicate))} \hlopt{+}
  \hlkwd{stat_ecdf}\hldef{(}\hlkwc{geom} \hldef{=} \hlsng{"step"}\hldef{,} \hlkwc{col} \hldef{=} \hlsng{'grey30'}\hldef{)} \hlopt{+}
  \hlkwd{stat_ecdf}\hldef{(}
    \hlkwc{data} \hldef{=} \hlkwd{data.frame}\hldef{(}\hlkwc{val} \hldef{= x,} \hlkwc{replicate} \hldef{=} \hlsng{'0'}\hldef{),}
    \hlkwc{geom} \hldef{=} \hlsng{'step'}\hldef{,} \hlkwc{col} \hldef{=} \hlsng{'red'}\hldef{,} \hlkwc{linewidth} \hldef{=} \hlnum{1.2}
  \hldef{)} \hlopt{+} \hlkwd{theme_bw}\hldef{()}
\end{alltt}
\end{kframe}
\end{knitrout}
...
{\centering \includegraphics[width=\maxwidth]{tmp/figure/unnamed-chunk-13-1} 
