...
\begin{frame}[allowframebreaks]{Notation and generalizations}
  ...
  \begin{exampleblock}{Normal model example}
    Suppose we observe the following data: , and we would like to fit a normal model to the data, assuming the data are iid. Then $x_1^* = 3.49$, $x_2^* = 2$, and so forth, and the model family depends on $\theta = (\mu, \sigma^2)$, and the model can be expressed as:
    \begin{align*}
