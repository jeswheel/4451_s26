...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Example: Suppose we toss the coin 10 times, observe 8 heads. Then $\hat{p} = 8/10$, and then $\sqrt{\hat{p}(1 - \hat{p}) / N} = \Sexpr{round(sqrt(0.8*0.2/10), 3)}$, and our point estimate is $0.8$, with a confidence interval of $0.8\pm 1.96 \times \Sexpr{round(sqrt(0.8*0.2/10), 3)} = (\Sexpr{round(0.8-1.96*sqrt(0.8*0.2/10), 2)}, \Sexpr{round(0.8+1.96*sqrt(0.8*0.2/10), 2)})$. Do we see a problem? 
  \end{itemize}
...
# lower_bound <- qbeta(0.025, n+1, N-n+1)  # lower bound
# upper_bound <- qbeta(0.975, n+1, N-n+1)  # upper bound
cat("95% Credible Interval for p: (", round(lower_bound, 2), ", ", round(upper_bound, 2), ")", sep = '')
@
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item \texttt{d<dist>}: evaluate the density of a distribution at a particular point. Example: If $\phi(x)$ is the density of a standard normal, then $\phi(2) = $ \texttt{dnorm(2, mean = 0, sd = 1)} =  \Sexpr{round(dnorm(2, mean = 0, sd = 1), 2)}.
